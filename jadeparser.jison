...
%{
	...
	function directCopy(text) {
		return new ASTNode('DirectCopy', text, null);
	}
%}
...
Literal
    :	NULLTOKEN { $$ = directCopy('null'); }
    |	TRUETOKEN { $$ = directCopy('true'); }
    |	FALSETOKEN { $$ = directCopy('false'); }
    |	NUMBER { $$ = directCopy($1); }
    |	STRING { $$ = directCopy(yytext); }
    |	'/' { $$ = directCopy('/'); }
    |	DIVEQUAL { $$ = directCopy('/='); }
    ;
...
Property
    :	IDENT ':' AssignmentExpr {
			$$ = new ASTNode('JSONProperty', directCopy($1), $3);
		}
	|	IDENT {
			$$ = new ASTNode('JSONProperty', directCopy($1), directCopy($1));
		}
	|	STRING ':' AssignmentExpr {
			$$ = new ASTNode('JSONProperty', directCopy($1), $3);
		}
    |	NUMBER ':' AssignmentExpr {
			$$ = new ASTNode('JSONNumberProperty', directCopy($1), $3);
...
/* bottom of the LHS hierarchy (e.g. x = 5) */
PrimaryExprNoBrace
    :	THISTOKEN {
			$$ = directCopy('this');
		}
...
MemberExpr
    ...
    |	MemberExpr '.' IDENT {
			$$ = new ASTNode('Property', $1, directCopy($3));
		}
	|	MemberExpr '::' IDENT {
			$$ = new ASTNode('Prototype', $1, directCopy($3));
		}
...
Arguments
    :	'(' ')' {
			$$ = new ASTNode('FunctionArguments', null, null);
		}
    |	'(' ArgumentList ')' {
			$$ = new ASTNode('FunctionArguments', $2, null);
		}
    ;
...
AssignmentExpr
	...
	|	DestructuredIdentList AssignmentOperator AssignmentExpr {
			$$ = new ASTNode('DestructuringAssignment', $1, $3);
		}
    ;
...
AssignmentExprNoIn
	...
	|	DestructuredIdentList AssignmentOperator AssignmentExprNoIn {
			$$ = new ASTNode('DestructuringAssignment', $1, $3);
		}		
    ;
...
VariableDeclarationList
    ...
    |	IDENT Initializer {
			$$ = [ new ASTNode('=', directCopy($1), $2) ];
		}
	|	DestructuredIdentList Initializer {
			$$ = [ new ASTNode('DestructuringAssignment', $1, $2) ];
		}
...
VariableDeclarationListNoIn
	...
	|	DestructuredIdentList InitializerNoIn {
			$$ = [ new ASTNode('DestructuringAssignment', $1, $2) ];
		}
    ...
    |	VariableDeclarationListNoIn ',' IDENT InitializerNoIn {
			var init = new ASTNode('=', directCopy($3), $4);
			if ($1 instanceof Array) {
				$1.push(init);
				$$ = $1;
			}
...
ConstDeclaration
    ...
    |	IDENT Initializer {
			$$ = new ASTNode('ConstAssignment', directCopy($1), $2);
		}
    ;
...
IterationStatement
    ...
    |	FOR '(' VAR VariableDeclarationListNoIn ';' ExprOpt ';' ExprOpt ')' Statement {
			...
			$$ = new ASTNode('ScopedFor', forInfo, $10);
		}
...
ContinueStatement
    ...
    |	CONTINUE IDENT ';' {
			$$ = new ASTNode('Continue', directCopy($2), null);
		}
...
BreakStatement
    ...
    |	BREAK IDENT ';' {
			$$ = new ASTNode('Break', directCopy($2), null);
		}
    |	BREAK IDENT error {
...
LabelledStatement
    :	IDENT ':' Statement {
			$$ = new ASTNode('Label', directCopy($1), $3);
		}
    ;
...
FunctionDeclaration
	...
	|	FUNCTION IDENT '!' '(' FormalParameterList ')' OPENBRACE FunctionBody CLOSEBRACE {
			...
			$$ = new ASTNode('FunctionDeclaration', funcInfo, $8);	
		}
    ;
